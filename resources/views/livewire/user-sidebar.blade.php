        ...
        <article class="relative flex-1">
            <div class="absolute inset-0">
                <div class="h-full border-gray-900" aria-hidden="true">
                    <x-dropdown-link href="{{ route('dashboard') }}" class="block py-2 text-gray-100 hover:bg-gray-800 hover:text-gray-100 {{ in_array(Route::currentRouteName(), ['dashboard', 'user.dashboard']) ? 'bg-black text-gray-100' : '' }}">
                        {{ __('Dashboard') }}
                    </x-dropdown-link>
                    <x-dropdown-link href="{{ route('profile.show') }}" class="block py-2 text-gray-100 hover:bg-gray-800 hover:text-gray-100 {{ Route::currentRouteName() == 'profile.show' ? 'bg-black text-gray-100' : ''}}">
                        {{ __('Profile') }}
                    </x-dropdown-link>
                    <!-- Authentication -->
